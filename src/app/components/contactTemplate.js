...
export default function ContactTemplate({
  backgroundImage = "/images/common/contact-background.webp",
  titleText = "Contact Us",
  subtitleText = "Do you have any questions!",
  descriptionUnderImage = "",
  contactDetails = {
    phone: "16930",
    address: "Building 340, South 90 St., New Cairo, Egypt",
    workHours: "From Sat to Thu, 10am to 8pm",
  },
  showMap = true,
  ...
  pageSource = "general",
  projectId = null,
  metaTitle = "Contact Us | Margins Developments",
  metaDescription = "Get in touch with Margins Developments",
}) {
  ...
  }, [setMetadata, searchParams, metaTitle, metaDescription, pageSource, projectId]);
    ...
    <div>
      ...
      <main className={styles.wrapper}>
        <div className="relative w-full overflow-hidden mt-[0px]">
          <Image src={backgroundImage} alt="contact-background" className={`${styles.banner} w-full`} width={535} height={525} quality={100} />
          <div className={`absolute left-0 right-0 top-0 bottom-0 flex flex-col justify-center mt-[30px] mb-[40px] max-w-[1400px] pl-5 pr-5 ml-auto mr-auto ${isMobileDevice ? "text-center" : ""}`}>
            ...
            <h1 className={`text-white font-bold uppercase ${isMobileDevice ? "w-[100%] text-[40px]" : "text-[80px] w-[440px]"}`}>{subtitleText}</h1>
          </div>
        </div>
        ...
        {descriptionUnderImage && (
          <div className="bg-white">
            <div className="max-w-[1400px] mx-auto px-[20px] py-[30px]">
              <p className="text-gray-700 text-[16px] leading-relaxed">{descriptionUnderImage}</p>
            </div>
          </div>
        )}
            ...
            <div className="w-full md:w-1/2 lg:w-2/5">
              ...
              >
                {contactDetails.phone && (
                  <motion.li className={`flex items-center ${styles.detail}`} variants={SlideVariants} transition={{ duration: 0.5 }}>
                    ...
                    <span className={styles.text}>{contactDetails.phone}</span>
                  </motion.li>
                )}
                {contactDetails.address && (
                  <motion.li className={`flex items-center ${styles.detail}`} variants={SlideVariants} transition={{ duration: 0.5 }}>
                    ...
                    <span className={styles.line}></span>
                    <span className={styles.text}>{contactDetails.address}</span>
                  </motion.li>
                )}
                {contactDetails.workHours && (
                  <motion.li className={`flex items-center ${styles.detail}`} variants={SlideVariants} transition={{ duration: 0.5 }}>
                    <div className={styles["detail-inner"]}>
                      <span className="mr-3">
                        <RxCalendar size={16} />
                    ...
                    <span className={styles.line}></span>
                    <span className={styles.text}>{contactDetails.workHours}</span>
                  </motion.li>
                )}
              </motion.ul>
            </div>
            ...
            <div className="w-full md:w-1/2 lg:w-2/5">
              ...
              {responseMsg && (
                <div className={`mt-4 p-3 rounded-md ${isSuccess ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}>
                  <p className={`text-center ${isSuccess ? "text-green-700" : "text-red-600"} font-medium`}>{responseMsg}</p>
                </div>
              )}
            </div>
          </div>
